...
export default function N4GrammarLesson27Page() {
  ...
  const saveNotes = () => {
    ...
    localStorage.setItem('n4-grammar-lesson27-notes', notes);
    setShowNotesModal(false);
  };

  const loadNotes = () => {
    // Tải ghi chú từ localStorage
    const savedNotes = localStorage.getItem('n4-grammar-lesson27-notes');
    if (savedNotes) {
      setNotes(savedNotes);
    }
    
    // Tải ghi chú cho từng ngữ pháp
    const savedGrammarNotes = localStorage.getItem('n4-grammar-lesson27-grammar-notes');
  ...
  const saveGrammarNote = (patternId: string, content: string) => {
    ...
    localStorage.setItem('n4-grammar-lesson27-grammar-notes', JSON.stringify(newGrammarNotes));
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/api/grammar/n4/lesson27');
        if (response.ok) {
          const data = await response.json();
          setLessonData(data);
        } else {
          // Fallback to static data
          const staticData = await import('../../../../../../../data/jlpt-n4/B27.json');
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mt-8 flex justify-between">
          ...
          <Link
            href="/courses/japanese/n4/grammar/lesson/28"
