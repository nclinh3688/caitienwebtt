...
import { N5Lesson3Grammar } from '../../../../../../../data/grammar/n5/lesson3';

export default function N5GrammarLesson3Page() {
  ...
  const saveNotes = () => {
    ...
    localStorage.setItem('n5-grammar-lesson3-notes', notes);
    setShowNotesModal(false);
  };

  const loadNotes = () => {
    // Tải ghi chú từ localStorage
    const savedNotes = localStorage.getItem('n5-grammar-lesson3-notes');
    if (savedNotes) {
      setNotes(savedNotes);
    }
    
    // Tải ghi chú cho từng ngữ pháp
    const savedGrammarNotes = localStorage.getItem('n5-grammar-lesson3-grammar-notes');
  ...
  const saveGrammarNote = (patternId: string, content: string) => {
    ...
    localStorage.setItem('n5-grammar-lesson3-grammar-notes', JSON.stringify(newGrammarNotes));
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/api/grammar/n5/lesson3');
        if (response.ok) {
          const data = await response.json();
          setLessonData(data);
        } else {
          // Fallback to static data
          setLessonData(N5Lesson3Grammar);
        }
      } catch (error) {
