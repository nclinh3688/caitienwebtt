...
export default function VocabularyLessonListPage() {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      try {
        // First, find the N5 Japanese course to get its ID
        const courseResponse = await fetch('/api/courses?language=japanese&level=n5');
        const courseData = await courseResponse.json();
        
        if (courseData.success && courseData.data.length > 0) {
          const n5Course: Course = courseData.data[0];
          ...
          setLessons(n5Course.lessons.sort((a, b) => a.order - b.order));
        } else {
          console.error('Could not find the N5 Japanese course.');
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        >
          ...
          <h1 className="text-4xl font-bold text-gray-800">Từ vựng JLPT N5</h1>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          <ul className="space-y-4">
            {lessons.map((lesson, index) => (
              ...
              >
                <Link href={`/courses/japanese/n5/vocabulary/${lesson.order}`}>
