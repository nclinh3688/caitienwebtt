...
export async function POST(request: NextRequest) {
  try {
    const { question, context, model = 'gemini-pro', maxTokens = 150, temperature = 0.7 } = await request.json();
    ...
    const response = result.response.text() || 'Xin lỗi, tôi không thể trả lời câu hỏi này.';
